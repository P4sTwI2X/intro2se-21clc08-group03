...
export default function Header() {
  ...
  const [cookie, _, removeCookie] = useCookies(["cart", "token", "fullname"]);

  const [len, setLen] = useState(!cookie.cart ? cartSample.length : (cookie.cart as Cart).arr.length);
  useEffect(() => {
    ...
    <nav className="bg-[#7A9D54] border-none dark:bg-gray-900 grid grid-rows-2 gap-0">
      <div className="w-full flex flex-wrap items-center justify-end px-10">
        <ul className="flex justify-between space-x-5">
          ...
          {cookie?.token != null ? (
            <>
              <h2 className="text-white font-bold">Hello, {cookie?.fullname}</h2>
              <li className="text-white font-light">|</li>
              <button className=" hover:text-yellow-400 text-red-700" onClick={logout}>
                Log out
              </button>
            </>
          ) : (
